...
| **Next Game card** | A home-screen primitive (`_NextGameCard` in `home_screen.dart`). Shows the user's nearest upcoming booking — venue, date·time·court, countdown, wedge-reinforcing footer. When no upcoming booking exists, renders the `_EmptyNextGame` CTA ("Ready to play? / Book"). It is the first thing below the header + search — treat it as the quick-book slot. | A separate route/screen. A booking history list. A calendar. |
...
// Pluralize count strings — never `$n items` unconditionally
'$n ${n == 1 ? 'court' : 'courts'}'
```
...
- Neutralize the `colorSport*` tokens (basketball orange, cricket mint, etc.) into a single grey. They are brand-locked — never collapse them to de-clutter a design. If a reviewer tells you the sport chips look "random pastel," they're wrong.
- Use `overline` / `labelS` grey for section headers. Always use `headingM` primary with the red accent bar (see `_SectionHeader` in `home_screen.dart`).
...
| Home | /home | ✅ Next Game card + Live Now + Map + Chips + Courts + Feed |
| Sport | /sport/:id | ✅ |
